        ...
        <div class="formBox">
            <div class="tabs">
                <div class="tabs-chose">
                    <li v-for="(paper, index) in testPaperContentData.Papers" :class="{active : currentTab === index }" @click="currentTab = index">
                        <p>{{paper.Name}}</p>
                    </li>
                </div>
                                ...
                                <div class="questionContent">
                                    <div v-for="(question, questionIndex) in section.Questions" class="questiones">
                                        ...
                                        <div v-for="(option, optionIndex) in question.Options" class="questionChoses">
                                            <input class="radioclass " type="radio" :disabled="isComplete" :value="option.Code"
                                            :id="option.ID"
                                            :name="question.ID"
                                            v-model="testPaperFormData.Papers[paperIndex].Sections[sectionIndex].Questions[questionIndex].Result"
                                            @change="questionOptionChecked($event)"
                                            @click="changeClear($event)">
        ...
        methods: {
            ...
            requesTestPaperData() {
                getPacientTestDetails(this.testID).then(res => {
                    ...
                    this.testPaperFormData = JSON.parse(JSON.stringify(res));
//                    this.testPaperFormData = Object.assign({}, res);
            ...
            JudgShowSummary(res) {
                ...
                let status = resultArr.length === res.Papers.length
                return  status
            },
            updateSummary() {
                let params = {
