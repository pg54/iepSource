<template>
    <div class="printWrap">
        <div class="print-sider onc-scrollbar">
            <h2 class="print-sider-title">{{ paperTitle}}</h2>

            <el-menu default-active="1" class="el-menu-vertical-demo" @open="handleOpen" @close="handleClose">

                <el-menu-item index="1" @click.native='choseRegister'>{{ '入学登记表' }}</el-menu-item>

                <el-submenu index="4">
                    <template slot="title">{{ '入学学习能力评估' }}</template>
                    <el-menu-item-group v-for="(item, itemIndex) in entrancelistData" :key='item.ID' :title="item.CreatedOn">
                        <el-menu-item :key="item.ID" :index="'4' + itemIndex + 1" @click.native="requestEntranceContent(item.ID, paperTitle + '学习能力评估表')">入学评测表</el-menu-item>
                        <el-menu-item :key="item.ID" :index="'4' + itemIndex + 2" @click.native="requestEntranceReport(item.ID, paperTitle + '学习能力综合分析报告')">入学评测评估报告</el-menu-item>
                    </el-menu-item-group>
                </el-submenu>

                <el-submenu index="2">
                    <template slot="title">{{ '康复课程评测' }}</template>
                    <el-menu-item-group v-for="(ele, index) in testPaperListData" :title="ele.PackageName">
                        <el-menu-item :index="'2' + (index + 1) + '-1'" @click.native="requestTestReportData(ele.ID, ele.PackageName + '康复课程报告')">{{ ele.PackageName + '康复课程报告'}}</el-menu-item>
                        <el-menu-item :index="'2' +(index + 1) + '-2'" @click.native="requesTestResultData(ele.ID, ele.PackageName + '康复课程评估表')">{{ ele.PackageName + '康复课程评估表'}}</el-menu-item>
                    </el-menu-item-group>
                </el-submenu>

                <el-submenu index="3">
                    <template slot="title">{{ '康复个性化教育计划（IEP）'}}</template>

                    <el-submenu v-for="(ele, el) in personalPlanData.Records" :index="'3-'+ (el + 1)">
                        <template slot="title">
                            <p @click="requestPlanDetails(ele.ID)">{{ ele.Title }}</p>
                        </template>
                        <el-menu-item :index="'3-'+ (el + 1)+ (index + 1)" v-for="(item, index) in planDetailsData.MonthPlans" @click="requestMonthDetails(item.ID, ele.Title, item.Name)">{{ item.Name }}</el-menu-item>
                    </el-submenu>
                </el-submenu>
                ...
                <el-submenu index="5">
                    ...
                    <el-menu-item-group v-for="(item, index) in cpepMemdbersData" :title="item.PatientAgentName + ' ' + item.CreatedOn.split('T')[0]">
                        <el-menu-item :index="'5' + (index + 1) + '-2'" @click="checkCPEPScore(index, item)">C-PEP-3计分册</el-menu-item>
                        <el-menu-item :index="'5' + (index + 1) + '-1'" @click="checkCPEPResult(index, item)">C-PEP-3分析图表</el-menu-item>
                    </el-menu-item-group>
                </el-submenu>

                <el-submenu index="13">
                    <template slot="title">感觉统合评定量表</template>
                    <el-menu-item-group v-for="(item, index) in cpepFeelingMemdbersData" :title="item.PatientAgentName + ' ' + item.CreatedOn.split('T')[0]">
                        <el-menu-item :index="'13' + (index + 1) + '-1'" @click="choseFeeling(index, item)">感觉统合计分册</el-menu-item>
                        <el-menu-item :index="'13' + (index + 1) + '-2'" @click="choseFeelingChart(index, item)">感觉统合分析图表</el-menu-item>
                    </el-menu-item-group>

                </el-submenu>

                <el-submenu index="14">
                    <template slot="title">孤独症儿童自理能力测评</template>
                    <el-menu-item-group v-for="(item, index) in cpepselfCareMemdbersData" :title="item.PatientAgentName + ' ' + item.CreatedOn.split('T')[0]">
                        <el-menu-item :index="'14' + (index + 1) + '-1'" @click="choseSelfCare(index, item)">孤独症儿童自理能力测评计分册</el-menu-item>
                    </el-menu-item-group>
                </el-submenu>

                <!-- <el-menu-item index="13" @click.native='choseFeeling'>感觉统合评定量表 </el-menu-item> -->
                <el-menu-item index="12" @click.native='choseAttachment'>{{ '患者材料'}}</el-menu-item>
        ...
        <div class="print-content">
            <div class="print-content-paper-wrapper onc-scrollbar">
                <div id="print-content-paper" v-loading.fullscreen.lock="printFullscreenLoading" element-loading-text="拼命加载中">
                    ...
                    <div class="paperform-content" v-if="paperIndex === 5">
                        <CPEPPaper :cpepChartData="cpepDevResult" :setContent='true'></CPEPPaper>
                    </div>
                    ...
                    <div class="paperform-content" v-if="paperIndex === 13">
                        <div style="position: relative;height: 675px">
                            <AttachUpload ref="attachmentChild"></AttachUpload>
                        </div>
                    </div>

                    <div class="paperform-content" v-if="paperIndex === 14">
                        <printFeeling :cpepData="cpepFeelingdata" :isResultPage="true"></printFeeling>
                    </div>

                    <!-- 感觉统合分析图表 -->
                    <div class="paperform-content" v-if="paperIndex === 16">
                        <cpepFeelingChart :cpepData="cpepFeelingdata" :isResultPage="true" :setContent='true'></cpepFeelingChart>
                    </div>

                    <div class="paperform-content" v-if="paperIndex === 15">
                        <printSelfCare :cpepData="cpepSelfCaredata" :isResultPage="true" :setContent='true'></printSelfCare>
                    </div>
                </div>
        ...
        <el-dialog title="提示" :visible.sync="dialogVisible" size="tiny" :before-close="handleClose">
            <a id='lodapA' href='http://www.lodop.net/download/CLodop_Setup_for_Win32NT_https_3.029Extend.zip' target='_blank'>打印插件未安装!点击确定执行安装</a>
...
<script>
    ...
    import printFeeling from '../components/printCPEPFeeling.vue'
    import cpepFeelingChart from '../components/printCPEPFeelingChart.vue'
    import printSelfCare from '../components/CPEPselfCare.vue'
    ...
    export default {
        ...
        computed: {
            // stuName: function () { //侧板栏不显示学生名称
            //     return this.$store.state.mystudent.currentPatientInfo.Name
            // },
            paperTitle: function () {
                return this.$store.state.printPaper.printPaperTitle
            }
        },
        data() {
            return {
                ...
                dialogVisible: false,
                cpepMemdbersData: '',
                cpepFeelingMemdbersData: '',
                cpepselfCareMemdbersData: '',
                CPEPResultData: '',
                pacientID: '',
                cpepTotalResult: '',
                cpepQuestionResults: '',
        ...
        methods: {
            ...
            CreateOneFormPage() {
                ...
                if (this.paperIndex === 0) {
                    ...
                    var printSth = styleCSS + "<body>" + document.getElementById("paperform-form2").innerHTML +
                        "</body>"
                    LODOP.PRINT_INIT("");
                ...
                } else if (this.paperIndex === 5) {  //CPEP3分析图表
                    styleCSS = "<link href='../../static/printCPEP.css' type='text/css' rel='stylesheet'>"
                    var page = document.getElementsByClassName("paperform-form")[0];
                    page.removeChild(page.children[1])
                    LODOP.PRINT_INIT("");
                    LODOP.SET_PRINT_PAGESIZE(1, 0, 0, "A4");
                    LODOP.ADD_PRINT_HTM('10', '40', '95%', '28.7cm', page.innerHTML);
                } else if (this.paperIndex === 6) { //CPEP3计分册
                    styleCSS = "<link href='../../static/printCPEP.css' type='text/css' rel='stylesheet'>"
                ...
                } else if (this.paperIndex === 14) {  //感觉统合计分册
                    styleCSS = "<link href='../../static/printCPEP.css' type='text/css' rel='stylesheet'>"
                    ...
                    // ADD_PRINT_IMAGE
                    LODOP.ADD_PRINT_HTM('10', '40', '95%', '28.7cm', page.innerHTML);
                } else if (this.paperIndex === 16) {  //感觉分析图表
                    var p = document.getElementsByClassName("paperform-form")[0]
                    p.removeChild(p.children[0])
                    LODOP.PRINT_INIT("");
                    LODOP.SET_PRINT_PAGESIZE(1, 0, 0, "A4");
                    LODOP.ADD_PRINT_HTM('10', '40', '95%', '28.7cm', p.innerHTML);
                    ...
                    LODOP.ADD_PRINT_HTM(26, "5%", "90%", 60, headerSth);
                    LODOP.ADD_PRINT_HTM(100, "5%", "90%", '24.7cm', printSth);
                } else if (this.paperIndex === 13) {
                    var imgPaths = this.$refs['attachmentChild'].$data.attachmentList.map(ele => ele.Path);
                    ...
                    LODOP.PRINT_INIT("");
                    LODOP.SET_PRINT_PAGESIZE(1, 0, 0, "A4");
                    LODOP.ADD_PRINT_HTM(26, "5%", "90%", '27cm', imgContainer.innerHTML);
                } else if (this.paperIndex === 15) {
                    var p = document.getElementsByClassName("paperform-form")[0]
                    p.removeChild(p.children[0])
                    LODOP.PRINT_INIT("");
                    LODOP.SET_PRINT_PAGESIZE(1, 0, 0, "A4");
            ...
            getLodop(oOBJECT, oEMBED) {
                var strHtmInstall =
                    "<br><font color='#FF00FF'>打印控件未安装!点击这里<a href='http://113.10.155.131/install_lodop32.zip' target='_self'>执行安装</a>,安装后请刷新页面或重新进入。</font>";
                var strHtmUpdate =
                    "<br><font color='#FF00FF'>打印控件需要升级!点击这里<a href='http://113.10.155.131/install_lodop32.zip' target='_self'>执行升级</a>,升级后请重新进入。</font>";
                var strHtm64_Install =
                    "<br><font color='#FF00FF'>打印控件未安装!点击这里<a href='http://113.10.155.131/install_lodop64.zip' target='_self'>执行安装</a>,安装后请刷新页面或重新进入。</font>";
                var strHtm64_Update =
                    "<br><font color='#FF00FF'>打印控件需要升级!点击这里<a href='http://113.10.155.131/install_lodop64.zip' target='_self'>执行升级</a>,升级后请重新进入。</font>";
                var strHtmFireFox =
                    "<br><br><font color='#FF00FF'>（注意：如曾安装过Lodop旧版附件npActiveXPLugin,请在【工具】->【附加组件】->【扩展】中先卸它）</font>";
                var strHtmChrome = "<br><br><font color='#FF00FF'>(如果此前正常，仅因浏览器升级或重安装而出问题，需重新执行以上安装）</font>";
                var strCLodopInstall =
                    "<p style='color: #ffffff;background-color: #1c8de0'>打印插件未安装!点击这里<span @click='jumpLogin'>执行安装</span><a id='lodapA' href='http://www.lodop.net/demolist/CLodopPrint_Setup_for_Win32NT.zip' target='_self'></a>,安装后重新登录。</p>";
                var strCLodopUpdate =
                    "<br><font color='#FF00FF'>CLodop云打印服务需升级!点击这里<a href='CLodopPrint_Setup_for_Win32NT.zip' target='_self'>执行升级</a>,升级后请刷新页面。</font>";
                var LODOP;
                try {
                    var isIE = (navigator.userAgent.indexOf('MSIE') >= 0) || (navigator.userAgent.indexOf('Trident') >=
                        0);
                    if (this.needCLodop()) {
                        try {
                            LODOP = getCLodop();
            ...
            requestEntrance() {
                let patientId = this.$store.state.mystudent.currentPatientInfo.ID;
                let params = {
            ...
            requestEntranceContent(id, title) {
                this.paperIndex = 4
                this.printFullscreenLoading = true;
                api.getPacientTestDetails(id).then(res => {
                    this.resultData = res;
                    this.$store.dispatch('setPrintPackageName', title);
                    this.printFullscreenLoading = false;
                })
            },
            requestEntranceReport(id, title) {
                this.paperIndex = 12
                this.printFullscreenLoading = true;
            ...
            registerRequest() {
                ...
                axios.all([api.getDictionaryData('FAMILYMODEL'), api.getDictionaryData('COMMUNITY'), api.getDictionaryData(
                        'EDUCATIONSTYLE'), api.getDictionaryData('LANGUAGE'), api.getDictionaryData(
                        'DEPENDENTS')])
                    .then(axios.spread(function (one, two, three, four, five) {
                        // Both requests are now complete
                        _this.fiveConditionData.push(one.data.List, two.data.List, three.data.List, four.data.List,
                            five.data.List)
                    }));
            },
            IEPRequest() {
                let params = {
                    ...
                    pageSize: 10,
                    planType: 1,
                    StudentID: this.$store.state.mystudent.currentPatientInfo.ID
                }
                api.drTeachingPlan(params).then(res => {
                    //res.data.Records = [],折叠菜单不展开
            ...
            choseFeeling(index, row) {
                ...
                api.getCPEPReport(params).then(res => {
                    this.cpepFeelingdata = res.data.Data.Results;
                    this.printFullscreenLoading = false;
                })
            },
            choseFeelingChart(index, row) {
                this.paperIndex = 16;
                this.printFullscreenLoading = true;
                let params = {
                    testId: row.ID,
            ...
            choseSelfCare(idnex, row) {
                this.paperIndex = 15;
                this.printFullscreenLoading = true;
                let params = {
                    testId: row.ID,
                    // PatientAgentID: row.PatientAgentID,
                    Type: 3 //孤独症儿童自理能力type是3
                };
                api.getCPEPReport(params)
                    .then(res => {
                        this.cpepSelfCaredata = res.data.Data.Results;
                        this.printFullscreenLoading = false;
                    })
                    .catch(err => {
                        this.printFullscreenLoading = false;
            ...
            testRequest() {
                ...
                let params = {
                    PageIndex: 1,
                    PageSize: 100,
                    PatientAgentID: _this.$store.state.mystudent.currentPatientInfo.ID
                };
                api.requestTestMembers(params).then(res => {
            ...
            requestTestReportData(id, title) {
                this.paperIndex = 1
                this.printFullscreenLoading = true
                api.getPacientTestReport(id).then(res => {
                    this.reportData = res
            ...
            checkTestedMembers() {
                //cpep评测结果
                let params = {
                    PatientAgentID: this.$store.state.mystudent.currentPatientInfo.ID,
                    Type: 1
                }
                api.getCPEPMember(params).then(res => {
                    this.cpepMemdbersData = res.data.Data.Records.filter(ele => ele.Status === 1);
                })
                //cpep 感觉统合结果
                let fparams = {
                    Size: 10,
                ...
                api.getCPEPMember(fparams).then(res => {
                    //Status1是已完成，0未完成，只显示未完成
                    this.cpepFeelingMemdbersData = res.data.Data.Records.filter(ele => ele.Status === 1);
                })
                //cpep 儿童自理能力结果
                let sparams = {
                    Size: 10,
                ...
                api.getCPEPMember(sparams).then(res => {
                    //Status1是已完成，0未完成，只显示未完成
                    this.cpepselfCareMemdbersData = res.data.Data.Records.filter(ele => ele.Status === 1);
                })
            ...
            requestMonthDetails(id, title, name) {
                ...
                api.getPlanItemDetail(id).then(res => {
                    ...
                    this.$store.dispatch('setPrintPaperMouth', name);
                    this.printFullscreenLoading = false;
                });
            }
        },
