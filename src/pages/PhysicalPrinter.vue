<template>
    <div class="printWrap">
        <div class="print-sider onc-scrollbar">
            ...
            <h2 class="print-sider-title">{{ '广东省' + paperTitle + '康复档案'}}</h2>

            <el-menu default-active="1" class="el-menu-vertical-demo" @open="handleOpen" @close="handleClose">

                <el-menu-item index="1" @click.native='choseRegister'>{{ paperTitle + '入学登记表'}}</el-menu-item>
                <el-submenu index="4">
                    <template slot="title">{{ paperTitle + '学习能力评估'}}</template>

                    <el-menu-item-group v-for="(item, itemIndex) in entrancelistData" :key='item.ID' :title="item.CreatedOn">
                        <el-menu-item :key="item.ID"  :index="'4' + itemIndex + 1" @click.native="requestEntranceContent(item.ID, paperTitle + '学习能力评估表')">{{ paperTitle + '学习能力评估表' }}</el-menu-item>
                        <el-menu-item :key="item.ID"  :index="'4' + itemIndex + 2" @click.native="requestEntranceReport(item.ID, paperTitle + '学习能力综合分析报告')">{{ paperTitle + '学习能力综合分析报告' }}</el-menu-item>
                    </el-menu-item-group>
                </el-submenu>

                <el-submenu index="2">
                    <template slot="title">{{ paperTitle + '康复课程评测'}}</template>
                ...
                <el-submenu index="3">
                    <template slot="title">{{ paperTitle + '康复个性化教育计划（IEP）'}}</template>
                ...
                <el-menu-item index="7" @click.native='choseClassRecords'>{{ paperTitle + '班级教学记录卡'}}</el-menu-item>

                <el-menu-item index="8" @click.native='choseSpecialRecords'>{{ paperTitle + '个别训练记录卡'}}</el-menu-item>

                <el-menu-item index="9" @click.native='choseStuTracking'>{{ paperTitle + '智力残疾儿童后续教育跟踪表'}}</el-menu-item>

                <el-menu-item index="10" @click.native='choseProgress'>{{ paperTitle + '学生学习进度报告表'}}</el-menu-item>

                <el-menu-item index="11" @click.native='choseSummary'>{{ paperTitle + '年度评估与总结'}}</el-menu-item>
                ...
                <el-menu-item index="12" @click.native='choseAttachment'>{{ paperTitle + '患者材料'}}</el-menu-item>
...
//                    var printSth = document.getElementsByClassName("paperform-form")[0].innerHTML
                    ...
                    let imgPaths = this.$refs['attachmentChild'].$data.attachmentList.map(ele => ele.Path);
