<template>
    <div class="paperform-form">
        <div id="canvasArea">
            ...
            <p style="text-align: center;font-size: 18px;font-weight: bold;width: 660px">儿童孤独症综合能力测评</p>
            <p style="text-align: center;font-size: 10px;width: 660px">（孤独症谱系及相关发育障碍儿童评估用心理教育量表C-PEP-3报告</p>
            <p class="chartText">
                测试者信息：
                <br> 
                编号：<span class="headTitle">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
                姓名：<span class="headTitle">{{ patientName}}</span>
                性别：<span class="headTitle">{{ patientSex === 1 ? '男': '女'}}</span>
                年龄：<span class="headTitle">
                    {{ calcDate(patientBirthday)}}
                </span>
            </p>
            <div>
                <p>测试结果：</p>
                ...
                <div>
                    <div style="margin-top: 20px;">
                        ...
                        <div class="projectScore">
                            <span>模仿</span>
                            <span>{{mfData.s}}分</span>
                            <span>{{mfData.t - 2}}-{{mfData.t + 2}}个月</span>
                        </div>
                        <div class="projectScore">
                            <span>知觉</span>
                    ...
                    <div id=blockChart>
                        ...
                        <div class="contentBox">
                            ...
                            <div class="contentItem">
                                ...
                                <div v-for="(el, index) in roundData[0].Result.A" :key="index + 20" class="ligcolor"></div>
                                <div v-for="(el, index) in roundData[0].Result.M" :key="index + 40" class="midcolor"></div>
                                <div v-for="(el, index) in roundData[0].Result.S" :key="index + 60" class="heavycolor"></div>
                                <div class="contenttIitle">{{ roundData[0].Domain }}</div>
                            </div>
                            <div class="contentItem">
                                <div v-for="(el, index) in 7" :key="index + 100" class="bgcolor"></div>
                                <div v-for="(el, index) in roundData[1].Result.A" :key="index + 120" class="ligcolor"></div>
                                <div v-for="(el, index) in roundData[1].Result.M" :key="index + 140" class="midcolor"></div>
                                <div v-for="(el, index) in roundData[1].Result.S" :key="index + 160" class="heavycolor"></div>
                                <div class="contenttIitle">{{ roundData[1].Domain }}</div>
                            </div>
                            <div class="contentItem">
                                <div v-for="(el, index) in 8" :key="index + 200" class="bgcolor"></div>
                                <div v-for="(el, index) in roundData[2].Result.A" :key="index + 220" class="ligcolor"></div>
                                <div v-for="(el, index) in roundData[2].Result.M" :key="index + 240" class="midcolor"></div>
                                <div v-for="(el, index) in roundData[2].Result.S" :key="index + 260" class="heavycolor"></div>
                                <div class="contenttIitle">{{ roundData[2].Domain }}</div>
                            </div>
                            <div class="contentItem">
                                <div v-for="(el, index) in roundData[3].Result.A" :key="index + 300" class="ligcolor"></div>
                                <div v-for="(el, index) in roundData[3].Result.M" :key="index + 320" class="midcolor"></div>
                                <div v-for="(el, index) in roundData[3].Result.S" :key="index + 340" class="heavycolor"></div>
                                <div class="contenttIitle">{{ roundData[3].Domain }}</div>
                            </div>
                            <div class="contentItem">
                                <div v-for="(el, index) in 3" :key="index + 400" class="bgcolor"></div>
                                <div v-for="(el, index) in roundData[4].Result.A" :key="index + 420" class="ligcolor"></div>
                                <div v-for="(el, index) in roundData[4].Result.M" :key="index + 440" class="midcolor"></div>
                                <div v-for="(el, index) in roundData[4].Result.S" :key="index + 460" class="heavycolor"></div>
        ...
        <el-row v-show="setContent" :gutter="20" id="teacherComment" style="border: 2px solid #118dee;border-radius: 4px;padding: 8px">
            <el-col :span="20">
                <el-row>
                ...
                <el-row>
                    <el-input
                        type="textarea"
                        :rows="4"
                        placeholder="模板：该儿童七大发展功能中，知觉、粗大动作的发展相对其他功能发展区域发展较好；模仿、知觉、精细动作、粗大动作、手眼协调 呈现发展空间；其中：认知表现、口语认知发展空间狭窄或发展相对（严重）滞后。在病理分析中，可呈现语言病理问题严重 程度突出；情感、人际关系、物品喜好、感知模式问题趋于中度水平。"
...
<script>
    ...
    export default {
        name: 'printCECP',
        props: ['cpepChartData', 'setContent'],
        data() {
            return {
                resultData: '',
                roundData: [],
        ...
        methods: {
            ...
            drawDevelopChart(lineData) {
                ...
                this.mfData = {
                    s: this.lineData[0].Result.P,
                    t: arr2[this.lineData[0].Result.P]
                }
                for (var i = 0; i < 11; i++) {
                    c.beginPath()
                ...
                this.zjData = {
                    s: this.lineData[1].Result.P,
                    t: arr3[this.lineData[1].Result.P]
                }
                for (var i = 0; i < 12; i++) {
                    c.beginPath()
                ...
                this.jxdzData = {
                    s: this.lineData[2].Result.P,
                    t: arr4[this.lineData[2].Result.P]
                }
                for (var i = 0; i < 11; i++) {
                    c.beginPath()
                ...
                this.cddzData = {
                    s: this.lineData[3].Result.P,
                    t: arr5[this.lineData[3].Result.P]
                }
                for (var i = 0; i < 12; i++) {
                    c.beginPath()
                ...
                this.syxtData = {
                    s: this.lineData[4].Result.P,
                    t: arr6[this.lineData[4].Result.P]
                }
                for (var i = 0; i < 15; i++) {
                    c.beginPath()
                ...
                this.rzbxData = {
                    s: this.lineData[5].Result.P,
                    t: arr7[this.lineData[5].Result.P]
                }
                for (var i = 0; i < 21; i++) {
                    c.beginPath()
                ...
                this.kyrzData = {
                    s: this.lineData[6].Result.P,
                    t: arr8[this.lineData[6].Result.P]
                }
                for (var i = 0; i < 21; i++) {
                    c.beginPath()
            ...
            calcDate(birStr) {
                let sy = Number(birStr.split('/')[0])
                let sm = Number(birStr.split('/')[1])
                let d = new Date()
                let y = d.getFullYear()
                let m = d.getMonth() + 1
                if (y - sy === 0) {
                    ...
                    if (m - sm > 0) {
                        return (y - sy) + '岁' + (m - sm) + '月'
                    } else {
                        return (y - sy - 1) + '岁' + (12 + m - sm) + '月'
                    }
                }
            },
            test() {
